...
$(function(){
	...
	function getDayData(month, start_dt, end_dt, work_seq) {
		...
		ajaxCallGet("/mng/statisticsbypartSearch.do?start_dt=" + start_dt
				+ "&end_dt=" + end_dt + "&month=" + month+"&work_seq"+work_seq, function(res) {
			...
			for (var i = 0; i < res.data.length; i++) {
				var partCode = res.data[i].partId;
	...
	function tbodyDataExcelFun(){
		global.htmlTableToExcel('tbodyDataExcelTable','부서별 통계','부서별 통계', "${EXCEL_SHEET_DATA_COUNT}");
	}
</script>
...
<script type="text/x-jsrender" id="selectRstList">
		{{for data}}
					<tr>
						<th rowspan="3"><span>{{:partId}}</span></th>
						<td><span>전송</span></td>
		...
		<div class="con-inner">
			...
			<h1 class="con-title">
				<ul>
					...
					<li><span class="dot"></span>부서별통계</li>
				</ul>
			</h1>
				...
				<div
					...
					<div
						...
						<input type="date"
							...
							name="start_dt" id="start_dt" placeholder="시작날짜" />
					</div>
					<div
					...
					<div
						...
						<input type="date"
							class="text-right font-174962 font-16px padding-0 margin-0 width-100 height-40px line-height-40px background-transparent"
							name="end_dt" id="end_dt" placeholder="종료일자"/ >
					</div>
				...
				<div
					...
					<div id="tableview">
						...
						<table class="con_tb_2 width-100" cellpadding="0" cellspacing="0"
							...
							<thead>
								<tr>
									<th class="width-10" rowspan="2"><span>부서명</span></th>

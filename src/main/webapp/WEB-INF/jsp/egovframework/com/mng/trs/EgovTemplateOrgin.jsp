...
$(document).ready(function(){
	...
	$("#tmpexcel").click(function(){
	    ...
	    global.htmlTableToExcel('template','템플릿','template', "${EXCEL_SHEET_DATA_COUNT}");
	});
});	
...
$(document).ready(function() {
	...
	/* $('#addTemplateBtn').on('click', function(){  
		...
		$('#addTemplateBtn').attr('disabled', true);		
		...
		//alert(JSON.stringify(obj));return;
     	...
     	$.ajax({
        	url: "<c:url value='/mng/insertTemplate.do' />",
        	type: "POST",
            data: obj,
            dataType: "json",
            beforeSend:function(){
                $('.wrap-loading').removeClass('display-none');
        		$('#addTemplateBtn').attr('disabled', false);
            },
			complete:function(){
		...
		drawCallback : function(settings) {
			...
			if (this_row > 0) {
				...
				for (var i = 0; i < rowCount; i++) {
					...
					td_body = td_body
							+ "<div class='column width-20 border-box padding-rl-10 margin-left-33 margin-right-33'><div class='width-100'>"
							+ "<h5 class='font-18px font-normal font-174962 text-left' style='display: inline-block;'>"
							+ rowData[i]['subject']
							+ "</h5><p class='font-15px font-normal' style='float: right;display: inline-block;"+tt+"' >"+status2+"</p>"
							+ "<div onclick='rowDetail("
							+ rowData[i]['template_data_seq']
							+ ");' class='width-100 height-340px font-4d4f5c font-16px text-left border-e8e8e8 border-radius-5 background-transparent padding-10 line-height-default margin-bottom-10'>"
							+ rowData[i]['template_content']
							+ "</div><a href='#'"+deleteBtn+" class='icon_delete margin-left-5' onclick='delRow("+rowData[i]['template_data_seq']+ ");' >삭제</a>"
							+ "</div>"
							+ "</div>";
		        ...
		        wb.SheetNames.forEach(function(sheetName){
		           //var toHtml = XLSX.utils.sheet_to_html(wb.Sheets[sheetName], { header: '' });
				    ...
				    for(var i in rowJsonObj){
						...
						var template_buttons = rowJsonObj[i]['버튼타입(웹링크, 앱링크, 배송조회, 봇키워드, 메시지 전달)'] + "," + rowJsonObj[i]['버튼명'] + "," + rowJsonObj[i]['링크'];
...
function rowDetail(template_data_seq) {
	...
	location.href = "/mng/templateDetail?tmpcode=" + template_data_seq;
}
...
<body>
			...
			<div class="con-inner">
				...
				<div  class="width-100 min-width-1110px margin-auto margin-top-30 margin-bottom-100 ">
					...
					<div class="width-100 height-70px padding-15 margin-bottom-30 background-f7fafc border-box box-shadow-small border-radius-5">
						...
						<div class="inline-block width-5px height-5px border-radius-5 background-d3e6ff"></div>
						<div class="inline-block width-5px height-5px border-radius-5 background-d3e6ff"></div>
						<div class="inline-block relative width-150px height-40px line-height-40px text-right  border-radius-5 border-8bc5ff border-box margin-right-5">
							<input type="date" id="end_dt" value="${today}" class="text-right font-174962 font-16px padding-0 margin-0 width-100 height-40px line-height-40px background-transparent" />
						</div>

						<!--사용여부 셀렉박스-->
						<select class="select_blank_lightblue width-100px" style="margin-right:10px">
						...
						<!--템플릿상태 셀렉박스-->
						<select class="select_blank_lightblue width-110px" style="margin-right:10px">
							<option selected>템플릿상태</option>
							<option>심사중</option>
							<option>검수반려</option>
							<option>등록거절</option>
							<option>승인완료</option>
						...
						<!--검색-->
						<div class="inline-block width-210px">
					...
					<div class="background-fff padding-15 margin-bottom-100 border-box box-shadow-small border-radius-5">
						<div class="table_result_tit">
							총 00/000개를 검색하였습니다.
							<a id="tmpexcel" class="icon_btn width-100px background-00e04e"><span class="icon_clip"></span>엑셀 다운로드</a>
							<a class="open_pop_templet margin-right-10 background-826fe8">템플릿등록</a>
						</div>
						...
						<div class="margin-top-20"><a href="#" class="icon_alldelete">전체삭제</a></div>
		 ...
		 <!-- popup 템플릿등록 -->
         <div class="pop_wrap pop_templet width-730px" style="background:transparent;">
			<div class="width-100">
				<ul class="pop_tabmenu_excel">
					...
					<li id="pop_tab_2" class="btnCon">
						<input type="radio" name="pop_tabmenu_excel" id="pop_tabmenu_2">
						<label for="pop_tabmenu_2">대량등록</label>
						<div class="pop_tabCon">
							<div class="width-100 margin-auto margin-top-20 margin-bottom-20 text-center font-16px font-000">
								...
								<ul class="pop_notice" style="padding-left:90px">
									...
									<li class="no-list">
										<font class="block margin-top-20" style="margin-bottom:7px;">* TIP *</font>
										<font class="block margin-bottom-5">엑셀파일의 전체 속성은 일반, 텍스트로 지정을 해야 하며 함수 등 연산이 입력되면 등록실패 처리 될 수 있습니다.</font>
										<font class="block margin-bottom-20">필수 항목에 공백이 있으면 등록이 실패할 수 있으므로 필수 정보는 모두 입력해주세요.</font>
									</li>
								</ul>
								<div class="filebox filebox_img bs3-primary text-left padding-left-90">
									<a class="width-140px height-30px line-height-30px text-center border-radius-5 background-053c72 font-12px font-fff box-shadow-smallest margin-left-5">엑셀양식 다운로드</a>
									<input class="upload-name" value="" disabled="disabled" placeholder="제목 (최대 30Byte)" />
									<label for="filetmp">찾아보기</label> 
									<input type="file" id="filetmp" class="upload-hidden" /> 
								</div>
							</div>
							<!--버튼-->
							<div class="pop_btn" style="margin-bottom:40px">
								<a class="pop_close background-8bc5ff border-8bc5ff font-fff" style=" width:120px; ">파일체크</a>
							</div>
							<div class="pop_btn">
								<a id="tmp_reg_excel" class="pop_close background-053c72 border-053c72 font-fff margin-right-20">등록</a>
								<a class="pop_close background-8bc5ff border-8bc5ff font-fff">취소</a>
							</div><br>
						</div>
					</li>
					<li id="pop_tab_3" class="btnCon">
						<input type="radio" name="pop_tabmenu_excel" id="pop_tabmenu_3">
						<label for="pop_tabmenu_3">승인된 템플릿 등록</label>
						<div class="pop_tabCon">
							<div class="width-100 margin-auto margin-top-20 margin-bottom-20 text-center font-16px font-000">
							...
							<div class="pop_btn">
								<a class="pop_close background-053c72 border-053c72 font-fff margin-right-20">등록</a>
								<a class="pop_close background-8bc5ff border-8bc5ff font-fff">취소</a>
							</div><br>
						</div>
					</li>

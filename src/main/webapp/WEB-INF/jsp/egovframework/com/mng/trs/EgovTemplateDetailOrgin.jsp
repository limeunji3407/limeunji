					...
					<table class="width-100 margin-bottom-40">
						...
						<tr>
							<c:if test="${workSeqRequiredFlag != 'ND'}">
								<th>업무분류</th>
								<td><select title="" id="work_seq" name="" required=""
									class="select_white width-260px">
										<c:if test="${workSeqRequiredFlag != 'Y'}">
											<option value="">선택해주세요</option>
										</c:if>
										<c:if test="${g_workSeqZeroCodeUseYN eq 'Y'}">
											<option value="0"<c:if test="${templateinfo.work_seq eq '0'}">selected</c:if>>미분류</option>
										</c:if>
										<option value="1"<c:if test="${templateinfo.work_seq eq '1'}">selected</c:if>>알림톡</option>
										<option value="2"<c:if test="${templateinfo.work_seq eq '2'}">selected</c:if>>알림</option>
										<option value="3"<c:if test="${templateinfo.work_seq eq '3'}">selected</c:if>>민원</option>
										<option value="4"<c:if test="${templateinfo.work_seq eq '4'}">selected</c:if>>공지</option>
										<option value="5"<c:if test="${templateinfo.work_seq eq '5'}">selected</c:if>>비상</option>
										<option value="6"<c:if test="${templateinfo.work_seq eq '6'}">selected</c:if>>기타</option>
								</select>
						...
						<tbody class="width-100">
							...
							<tr>
								<td class="padding-right-10 border-box"><input type="text"
									id="" name="" required="" readonly="readonly"
									class="padding-rl-20 border-box text-center width-130px"
									value="2020-00-00" /></td>
								<td class="padding-right-10 border-box"><input type="text"
									id="" name="" required="" readonly="readonly"
									class="padding-rl-20 border-box text-center" value="부서" /></td>
								<td class="padding-right-10 border-box"><input type="text"
									id="" name="" required="" readonly="readonly"
									class="padding-rl-20 border-box text-center" value="작성자" /></td>
								<td class="border-box"><input type="text" id="" name=""
									required="" readonly="readonly"
									class="padding-rl-20 border-box text-center width-230px"
									value="내용이 들어갑니다." /></td>
							</tr>
							<tr>
								<td class="padding-right-10 border-box"><input type="text"

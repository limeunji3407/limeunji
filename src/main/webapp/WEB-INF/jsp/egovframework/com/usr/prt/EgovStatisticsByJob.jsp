...
<script type="text/javascript"
	src="<c:url value='/js/egovframework/com/plugin/jqplot/plugin/jquery.barRenderer.js' />"></script>
<script type="text/javascript"
	src="<c:url value='/js/egovframework/com/plugin/jqplot/plugin/jquery.pointLabels.js' />"></script>
<script type="text/javascript"
	src="<c:url value='/js/egovframework/com/plugin/jqplot/plugin/jquery.categoryAxisRenderer.js' />"></script>
<script type="text/javascript"
	src="<c:url value='/js/egovframework/com/plugin/d3/d3.min.js' />"></script>
<link type="text/css" rel="stylesheet"
	href="<c:url value='/js/egovframework/com/plugin/jqplot/jquery.jqplot.min.css' />">
<!-- Load c3.css -->
...
$(function(){
	...
	function searchVaild() {
		...
		getDayData(monthMin, start_dt.value, end_dt.value);

	}
	function getDayData(month, start_dt, end_dt) {
		
		var userId = $("#userOpt").val();
		ajaxCallGet("/usr/partStatisticsbyjobSearch.do?start_dt=" + start_dt + "&user_id="+userId
				+ "&end_dt=" + end_dt + "&month=" + month, function(res) {
...
<script type="text/x-jsrender" id="selectRstList">
		{{for data}}
					<tr>
						<th rowspan="3"><span>{{:workNm}}</span></th>
						<td><span>전송</span></td>
	...
	<div class="con-inner">
		...
		<h1 class="con-title">
			<ul>
				...
				<li><span class="dot"></span>부서업무별통계</li>
			</ul>
		</h1>
		...
		<div
			...
			<div
				...
				<c:if test="${loginVO.orgnztId eq 'ORGNZT_0000000000000' or loginVO.partg_role eq 'Y' }">
					<select id="userOpt" class="select_blank_lightblue width-140px"

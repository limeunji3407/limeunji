...
var globalId = "${loginVO.id}";
		...
		drawCallback : function(settings) {
			...
			if (this_row > 0) {
				...
				for (var i = 0; i < rowCount; i++) {
					...
					if(status != 'APR' && status != 'DEL' && rowData[i]['status']!='R' && rowData[i]['process_ing']=='N'){
						deleteBtn = '';
					}else {
						deleteBtn = 'hidden';
					}
					...
					if(status == 'REJ' && rowData[i]['cur_status']=='500' && rowData[i]['process_ing']=='N'){
						modBtn = '';
					}else {
						modBtn = 'hidden';
					}
					...
					if((status == 'REJ' || status == 'REQ' || status == 'APR' ) && rowData[i]['cur_status']=='100' && rowData[i]['process_ing']=='N' && rowData[i]['status']!='R'){
						iquiry = '';
					}else {
						iquiry = 'hidden';
					}
					...
					td_body = td_body
							...
							+ rowData[i]['template_data_seq']
							+ ");' >삭제</a><a "+modBtn+" class='icon_modify margin-left-5' onclick='rowUpdate("
							+ rowData[i]['template_data_seq']
							+ ");' >수정</a><a "+iquiry+" class='icon_modify margin-left-5' onclick='rowIquiry("
							+ rowData[i]['template_data_seq']
							+ ");' >문의</a>"
							+ "</div>"
							+ "</div>";
	...
	function rowUpdate(template_data_seq) {
		console.log(template_data_seq);
		location.href = "/usr/templateUpdate?tmpcode=" + template_data_seq;
	}
	
	function rowIquiry(template_data_seq) {
		console.log(template_data_seq);
		$("#rowIquiryTemplate_code").val(template_data_seq);
		$(".pop_bg").css('display', 'block');
		$('#pop_rowIquiry').css('display','block');
	}
...
var arrTypeHtml = [
	...
	function iquiryForm() {
		$('#rowIquiry-form').parsley().validate();
		if (!$('#rowIquiry-form').parsley().isValid()) {
			return;
		}
		
		var template_code = $("#rowIquiryTemplate_code").val(); //text
		var content = $("#iquiry_content").val();
		
		var obj = {
            	"template_data_seq": template_code,
            	"content": content
		};
...
<body>
	...
	<div class="con-inner">
		...
		<div
			...
			<div
				...
				<p style="display:inline-block; font-size:8px; float:right; margin-top:30px;">상태가 [처리중], [심사중], [승인완료] 인 건은 삭제할 수 없습니다.</p>
			</div>
			<!-- //검색설정 -->
	...
	<div id="pop_rowIquiry" class="pop_wrap width-430px">
		<form id="rowIquiry-form" data-parsley-validate="">
			<div class="pop_tit">문의하기</div>
			<input type="text" id="rowIquiryTemplate_code" hidden />
			<div class="width-100 padding-bottom-10 border-bottom-e9eced">
				<table class="width-100" cellpadding="0" cellspacing="0" border="0">
					<tr>
						<th class="width-100px font-000 font-16px text-left">문의(댓글) 내용</th>
						<td class="text-left"><input type="text" name="iquiry_content" id="iquiry_content"
			...
			<!-- 버튼 -->
			<div class="pop_btn">
				<a class="background-8bc5ff border-8bc5ff font-fff margin-right-20" id="iquiryBtn"
				onclick="iquiryForm()">문의</a>
				<!-- <input type="submit" id="iquiryBtn"
					class="background-8bc5ff border-8bc5ff font-fff margin-right-20"
					onclick="iquiryForm()" value="문의"> -->

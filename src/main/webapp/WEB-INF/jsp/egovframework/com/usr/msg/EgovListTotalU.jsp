...
$(document).ready(function() {	
	...
	//엑셀 다운로드
	$("#listallExlBtn").click(function(){
		//테이블ID, sheet이름, 파일명+날짜.xlsx
	    //exportExcel('listall', '전체전송내역' ,'전체전송내역');
	    global.htmlTableToExcel('listall','전체전송내역','전체전송내역', "${EXCEL_SHEET_DATA_COUNT}");
	});
	
	var oTable = $('#listall').DataTable({
	  	dom: "frtip",
	    ajax: {
	    	"url" : "<c:url value='/usr/listallcont.do' />"
	    },
	    columns: [ 
	    	{ data: "rcv_id" },
	    	 { 
	...
	$('#searchBtn').on( 'click', function () {
		...
		var start_dt = document.getElementById("start_dt");
 		...
 		console.log(obj);
 		oTable.ajax.url('/usr/listallcont.do').load();
		...
		if($("#filterOpt option:selected").val()){
			oTable.columns( $("#filterOpt option:selected").val()  ).search( $("#searchKeyword").val() )
		}
		console.log("columns :: "+$("#filterOpt option:selected").val() + "   search :: "+$("#searchKeyword").val());
		oTable.draw(); 
	...
	$.fn.dataTable.ext.search.push(
   	    function( settings, data, dataIndex ) {
   	    	if($("#start_dt").val() && $("#end_dt").val()){
    	        ...
    	        var tableDate = data[2] || 0; tableDate = global.replaceNumber(tableDate); tableDate = global.parse(tableDate);
	...
	$('#listall tbody').on('click', ':nth-child(7)', function(e) {
   	...
   	$('#listall tbody').on('click', ':nth-child(8)', function(e) {
		var rowData = oTable.row(this).data();
		var content = failStatus(rowData["rslt_code2"]);
		$("#tooltip").text(content).animate({ left: e.pageX, top: e.pageY }, 1);
		if (!$("#tooltip").is(':visible')) $("#tooltip").show()
   	})
   	
   	$('#listall tbody').on('click', ':nth-child(10)', function(e) {
		var rowData = oTable.row(this).data();
		var content = failStatus(rowData["rslt_code2"]);
		$("#tooltip").text(content).animate({ left: e.pageX, top: e.pageY }, 1);
		if (!$("#tooltip").is(':visible')) $("#tooltip").show()
    ...
    $('#listall tbody').on('mouseleave', function(e) {
    	  $("#tooltip").hide()
    })
    
    $('#listall tbody').on('click', ':nth-child(5)', function(e) {
		var rowData = oTable.row(this).data();
	...
	<div class="con-inner">
		...
		<h1 class="con-title">
			<ul>
				...
				<li><span class="dot"></span>전체전송내역</li>
			</ul>
		</h1>
		...
		<div
			...
			<div
				...
				<!-- 구분 셀렉박스-->
				<select id="timeOpt" class="select_blank_lightblue width-70px"
				...
				<!--유형 셀렉박스-->
				<select id="typeOpt" class="select_blank_lightblue width-125px"
					style="margin-right: 10px">
				...
				<!--상태 셀렉박스-->
				<select id="statueOpt" class="select_blank_lightblue width-70px"
					style="margin-right: 10px">
				...
				<div
					...
					<input type="date" id="start_dt" name="start_dt" value=""
				...
				<select id="filterOpt" class="select_blank_lightblue width-180px"
					...
					<option value="5">메시지제목</option>
					<option value="6">메시지내용</option>
				</select>


				<!--검색-->
				<div class="inline-block width-150px">
					<input id="searchKeyword" type="text" name="" placeholder="" id="searchKeyword"
			...
			<div
				...
				<div id="tooltip"></div>
				<table id="listall" class="con_tb" cellpadding="0" style="width: 100%;"
					cellspacing="0" border="0">
					<thead>
						<tr>
							<th class="width-5">구분</th>
							<th class="width-5">유형</th>
							<th class="width-12">전송일시</th>
							<th class="width-11">발신번호</th>
							<th class="width-11">수신번호</th>
							<th class="width-13">메세지 제목</th>
							<th class="width-20">메세지 내용</th>
							<th class="width-7">상태</th>

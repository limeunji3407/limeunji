...
$(document).ready(function() {
	...
	//엑셀 다운로드
	$("#lbyExlBtn").click(function(){
		//테이블ID, sheet이름, 파일명+날짜.xlsx
	    //exportExcel('listallbytype', '발송단위별전송' ,'발송단위별전송');
		global.htmlTableToExcel('listallbytype','발송단위별전송','발송단위별전송', "${EXCEL_SHEET_DATA_COUNT}");
	});
	
	 var oTable = $('#listallbytype').DataTable({
		 dom: "frtip",
	     ajax:{
		    	"url" : "<c:url value='/usr/getlistall.do' />",
	...
	$('#searchBtn').on( 'click', function () {
		...
		var start_dt = document.getElementById("start_dt");
 		...
 		console.log(obj);
 		oTable.ajax.url('/usr/getlistall.do').load();
	...
	<div class="con-inner">
		...
		<div
			...
			<div
				...
				<table id="listallbytype" class="con_tb">
					<thead>
						<tr>

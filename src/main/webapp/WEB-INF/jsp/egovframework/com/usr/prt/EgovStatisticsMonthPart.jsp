...
<script>
 	...
 	function getMonthData(param){
 		ajaxCallPost("/usr/partMonthstatistics.do", param, function(res){
	...
	function tbodyMonthExcelFun(){
		global.htmlTableToExcel('tbodyMonthExcelTable','부서기간별 통계(월별)','부서기간별 통계(월별)', "${EXCEL_SHEET_DATA_COUNT}");
	}
 </script>
</head>
<body>
	...
	<div class="tabCon">
		...
		<div class="tabmenu_search">
			<c:if test="${loginVO.orgnztId eq 'ORGNZT_0000000000000' or loginVO.partg_role eq 'Y' }">
				<select id="userOpt2" class="select_blank_lightblue width-140px"
			...
			<!-- 업무분류 셀렉박스-->
			<select class="select_blank_lightblue width-120px type-select-month"
				style="margin-right: 10px">
			</select>
			...
			<div
				...
				<input type="month"
					...
					name="month_start_dt" id="month_start_dt" placeholder="시작날짜" />
			</div>
			<div
			...
			<div
				...
				<input type="month"
					class="text-right font-174962 font-16px padding-0 margin-0 width-100 height-40px line-height-40px background-transparent"
					name="month_end_dt" id="month_end_dt" placeholder="종료일자"/ >
			</div>
		...
		<div
			...
			<div class="btn-right">
				<a href="javascript:tbodyMonthExcelFun()" class="icon_btn width-100px background-00e04e"><span

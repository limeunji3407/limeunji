...
<spring:message code="g_footerUes" var="g_footerUes" /> 
<spring:message code="bnr_warning" var="bnr_warning" /> 
<spring:message code="manualconf" var="manualconf" /> 
<spring:message code="bnr_cong" var="bnr_cong" /> 
<spring:message code="bnr_link" var="bnr_link" /> 
<!DOCTYPE html>
<html lang="ko">
<head>
...
<!-- bottom -->
<c:if test="${bnr_warning eq 'Y'}" >
<div id="bnr_warning"></div>
</c:if>



<c:if test="${bnr_cong eq 'Y'}" >
<script>
		$(function(){
			$.ajax({
		       	url: "/getBannerList.do",
		       	type: "POST",
		        dataType: "json",
		       	success: function(jsondata){  
		       		...
		       		if(jsondata != null && jsondata.data != null && jsondata.data.length > 0){
		       			for(var i = 0 ; i < jsondata.data.length ; i++){
		       				if(jsondata.data[i].reflctAt == 'Y'){
		       					var str = '\
				    				<div class="width-100 margin-auto margin-top-20 margin-bottom-20 text-center font-16px font-ff6464 line-height-20px" >\
				    					<a href="'+jsondata.data[i].linkUrl+'" target="_blank"><img src="'+jsondata.data[i].bannerImage+'" style="max-height:50px;"></a>\
				    				</div>\
				    			';
				    			$(".pop_banner div:last-child").after(str);
		       				}
		       			}
	...
	<div class="pop_banner" style="display:block; width:250px;"">
		<c:if test="${manualconf eq 'Y' }">
			<div class="width-100 margin-auto margin-top-20 margin-bottom-20 text-center font-16px font-ff6464 line-height-20px" >
				<a href="<c:out value='${bnr_link}'  />" target="_blank">메뉴얼 다운로드</a>
			</div>
		</c:if>
...
<c:if test="${g_footerUes eq 'Y'}" >
<footer>
	<div class="footer-inner" >
		인천광역시서구| 주소: 인천광역시서구서곶로307(심곡동) | 대표전화: 1234-5678 ㅣ<a href="/agreeU">개인정보처리방침</a><br>

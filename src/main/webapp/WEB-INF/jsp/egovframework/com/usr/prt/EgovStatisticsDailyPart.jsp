...
<script>
 	...
 	function getDayData(month, start_dt, end_dt){
 		...
 		var userId = $("#userOpt1").val();
 		
 		ajaxCallGet("/usr/partDailystatistics.do?start_dt="+start_dt+"&end_dt="+end_dt+"&month="+month+"&work_seq="+work_seq+"&user_id="+userId, function(res){
	...
	$(function(){
		...
		ajaxCallGetNoParse("/getlistallUall.do", function(res){
        	...
        	$("#userOpt1").html(htmlOutput);
        	$("#userOpt2").html(htmlOutput);
        	$("#userOpt3").html(htmlOutput);
		})
	})
	function tbodyDataExcelFun(){
		global.htmlTableToExcel('tbodyDataExcelTable','부서기간별 통계(일별)','부서기간별 통계(일별)', "${EXCEL_SHEET_DATA_COUNT}");
	}
 </script>
	...
	<div class="tabCon">
		...
		<div class="tabmenu_search">
			<c:if test="${loginVO.orgnztId eq 'ORGNZT_0000000000000' or loginVO.partg_role eq 'Y' }">
				<select id="userOpt1" class="select_blank_lightblue width-140px"

...
public class EgovLoginLogController {
	...
	@RequestMapping(value="/mng/loginlog.do")
...
*/
		...
		return "egovframework/com/mng/log/EgovLoginLog";
	}

	@RequestMapping(value="/getLoginLog")
	public String selectLoginLogInAjax(@ModelAttribute("searchVO") LoginLog loginLog,
			ModelMap model) throws Exception{
		...
		loginLog.setRecordCountPerPage(9999);
		HashMap<?, ?> _map = (HashMap<?, ?>)loginLogService.selectLoginLogInf(loginLog);
		int totCnt = Integer.parseInt((String)_map.get("resultCnt"));

		System.out.println("getLoginLog----" + _map.get("resultList"));
